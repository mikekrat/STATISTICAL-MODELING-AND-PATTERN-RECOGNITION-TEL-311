...
title('Normalized and dimension directed data ');
hold off;
...
figure(3)
plot(X_norm(:, 1), X_norm(:, 2), 'bo');
title('Normalized dataset returned from myPCA');
...
subplot(1, 2, 1)
displayData(X(1:100, :));
title('First 100 faces in the dataset');
...
subplot(1, 2, 2)
displayData(U(:, 1:36)');
title('Top 36 eigenvectors found');
...
str = sprintf(['Original faces for K = ', num2str(K)]);
title(str);
axis square;
...
str = sprintf(['Recovered faces for K = ', num2str(K)]);
title(str);
axis square;
